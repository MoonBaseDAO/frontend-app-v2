...
import { yoktoNear } from '@/utils';
import { Dialog, Transition } from '@headlessui/react';
import { InformationCircleIcon } from '@heroicons/react/24/outline';
import { FixedNumber } from 'ethers';
import { Dispatch, Fragment, SetStateAction, useEffect, useRef } from 'react';

interface AddProposalModalInterface {
  addr: string;
  proposalBond: number;
  open: boolean
...
export const AddProposalModal = ({ addr, proposalBond, open, setOpen }: AddProposalModalInterface) => {
  const cancelButtonRef = useRef(null)
  const { getDaoContract } = useNear();
  const councilRef = useRef<any>(null);
